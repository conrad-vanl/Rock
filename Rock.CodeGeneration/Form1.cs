...
namespace Rock.CodeGeneration
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void btnLoad_Click( object sender, EventArgs e )
        {
            ...
            tbGraphQLFolder.Text = Path.Combine( RootFolder().FullName, "GraphQL", "Types" );
            ...
            if ( cblModels.CheckedItems.Count > 0 )
            {
                ...
                if ( rootFolder != null )
                {
                    foreach ( object item in cblModels.CheckedItems )
                    {
                        ...
                        // only generate Service, REST, and GraphQL for IEntity types
                        if ( typeof( Rock.Data.IEntity ).IsAssignableFrom( type ) )
                        {
                            ...
                            if ( cbGraphQL.Checked )
                            {
                                WriteGraphQLFile( serviceFolder, type );
                            }
                        }
...
";
        ...
        /// <summary>
        /// Writes the GraphQL Type file for a given type
        /// </summary>
        /// <param name="rootFolder"></param>
        /// <param name="type"></param>
        private void WriteGraphQLFile( string rootFolder, Type type )
        {
            ...
            sb.AppendFormat( "namespace {0}" + Environment.NewLine, typeNamespace );
            sb.AppendLine( "{" );
            sb.AppendLine( "    /// <summary>" );
            sb.AppendFormat( "    /// {0} GraphQL Type" + Environment.NewLine, name );
            sb.AppendLine( "    /// </summary>" );
            sb.AppendFormat( "    public partial class {0} : ModelGraphType<{1}.{2}>" + Environment.NewLine, name, type.Namespace, type.Name );
            sb.AppendLine( "    {" );
            ...
            foreach ( var field in dataMembers.Where(x => x.Name != "Id" ) )
            {
                ...
                {
                    nullable = true;
                    fieldType = field.PropertyType.GetGenericArguments()[0];
            ...
            sb.AppendFormat("       public override {0}.{1} GetById(int id, GraphQLContext context)" + Environment.NewLine, type.Namespace, type.Name);
            sb.AppendLine("       {");
            sb.AppendFormat("           var service = new {0}.{1}Service(context.db);" + Environment.NewLine, type.Namespace, type.Name);
            sb.AppendLine("           return service.Get(id);");
            sb.AppendLine("       }");
            sb.AppendLine( "   }" );
            sb.AppendLine( "}" );

            var filePath1 = Path.Combine( rootFolder, "GraphQL", "Types" );
            var file = new FileInfo( Path.Combine( filePath1, "CodeGenerated", name + "Type.cs" ) );
            WriteFile( file, sb );
        }
...
" );
        ...
        private void tbGraphQLFolder_MouseDoubleClick( object sender, MouseEventArgs e )
        {
            fbdGraphQLOutput.SelectedPath = tbGraphQLFolder.Text;
            if ( fbdGraphQLOutput.ShowDialog() == DialogResult.OK )
            {
                tbGraphQLFolder.Text = fbdGraphQLOutput.SelectedPath;
            }
        }
