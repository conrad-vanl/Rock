//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the Rock.CodeGeneration project
//     Changes to this file will be lost when the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
// <copyright>
// Copyright by the Spark Development Network
//
// Licensed under the Rock Community License (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.rockrms.com/license
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// </copyright>
//

using GraphQL;
using Rock.Model;

namespace Rock.GraphQL.Types
{
    /// <summary>
    /// AnalyticsDimFinancialAccount GraphQL Type
    /// </summary>
    public partial class AnalyticsDimFinancialAccount : ModelGraphType<Rock.Model.AnalyticsDimFinancialAccount>
    {
       public AnalyticsDimFinancialAccount(): base("AnalyticsDimFinancialAccount")
       {
          Field("AccountId", x => x.AccountId, nullable: false);
          Field("AccountType", x => x.AccountType, nullable: false);
          Field("ActiveStatus", x => x.ActiveStatus, nullable: false);
          Field("CampusName", x => x.CampusName, nullable: false);
          Field("CampusShortCode", x => x.CampusShortCode, nullable: false);
          Field("Description", x => x.Description, nullable: false);
          Field("EndDate", x => x.EndDate, nullable: true);
          Field("ForeignGuid", x => x.ForeignGuid.ToStringSafe(), nullable: true);
          Field("ForeignKey", x => x.ForeignKey, nullable: false);
          Field("GlCode", x => x.GlCode, nullable: false);
          Field("ImageBinaryFileId", x => x.ImageBinaryFileId, nullable: true);
          Field("ImageUrl", x => x.ImageUrl, nullable: false);
          Field("Name", x => x.Name, nullable: false);
          Field("Order", x => x.Order, nullable: false);
          Field("ParentAccountId", x => x.ParentAccountId, nullable: true);
          Field("PublicDescription", x => x.PublicDescription, nullable: false);
          Field("PublicName", x => x.PublicName, nullable: false);
          Field("PublicStatus", x => x.PublicStatus, nullable: false);
          Field("StartDate", x => x.StartDate, nullable: true);
          Field("TaxStatus", x => x.TaxStatus, nullable: false);
          Field("Url", x => x.Url, nullable: false);
          Field("Guid", x => x.Guid.ToStringSafe(), nullable: false);
          Field("ForeignId", x => x.ForeignId, nullable: true);
       }
       public override Rock.Model.AnalyticsDimFinancialAccount GetById(int id, GraphQLContext context)
       {
           var service = new Rock.Model.AnalyticsDimFinancialAccountService(context.db);
           return service.Get(id);
       }
   }
}
